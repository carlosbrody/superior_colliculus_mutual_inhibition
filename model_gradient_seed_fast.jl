...
function run_dynamics( ;target1=1.0,target2=1.0,target3=1.0,target4=1,target5=0,target6=1,
    ...
    for i in [1:niter;]
        ...
        out = DiffBase.GradientResult(params2)  # out must be same length as whatever we will differentiate w.r.t.
        keyword_gradient!(out, (;pars...) -> run_dynamics(;random_seed=i,ntrials=50,pars...)[1], params1, params2)  # note initial values must be floats
        grad = DiffBase.gradient(out)
        # cost    = DiffBase.value(out)
