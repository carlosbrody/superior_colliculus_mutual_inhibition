...
echo "gcloud compute ssh  --zone=us-east1-c --command=cd $rootdir/; ls $argv[2] carlosbrody@$argv[1] | sort | uniq > temp_remote.txt"
gcloud compute ssh  --zone="us-east1-c" --command="cd $rootdir/; ls $argv[2]" carlosbrody@$argv[1] | sort | uniq > temp_remote.txt
...
# echo "gcloud compute scp  --zone=us-east1-c temp_newfiles.txt carlosbrody@$argv[1]: $rootdir/$argv[2]"
gcloud compute scp --zone="us-east1-c" temp_newfiles.txt "carlosbrody@$argv[1]":$rootdir/$argv[2]

# ssh into remote machine and tar up the files  then clean up by deleting the list after having used it
echo "gcloud compute ssh  --zone=us-east1-c --command=cd $rootdir/; cd $argv[2] ; tar cvfz temp_tarball.tz -T temp_newfiles.txt ; \rm temp_newfiles.txt carlosbrody@$argv[1]"
gcloud compute ssh  --zone="us-east1-c" --command="cd $rootdir/; cd $argv[2] ; tar cvfz temp_tarball.tz -T temp_newfiles.txt ; \rm temp_newfiles.txt" carlosbrody@$argv[1]

# Copy the tarball to the local machine
echo "gcloud compute scp  --zone=us-east1-c carlosbrody@$argv[1] :$rootdir/$argv[2]/temp_tarball.tz $argv[2]"
gcloud compute scp  --zone="us-east1-c" "carlosbrody@$argv[1]":$rootdir/$argv[2]/temp_tarball.tz $argv[2]
...
# Clean up remotely
echo "gcloud compute ssh  --zone=us-east1-c --command cd $rootdir/$argv[2] ; \rm temp_tarball.tz carlosbrody@$argv[1]"
gcloud compute ssh  --zone="us-east1-c" --command "cd $rootdir/$argv[2] ; \rm temp_tarball.tz" carlosbrody@$argv[1]
