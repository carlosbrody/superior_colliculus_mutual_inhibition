...
test_func =  (;params...) -> JJ_opto_plot(100,100; rule_and_delay_periods=F["rule_and_delay_periods"], theta1=model_params[:theta1], theta2=model_params[:theta2], post_target_periods=F["post_target_periods"], seedrand=F["test_sr"], cbeta=F["cb"], verbose=true,plot_conditions=[false,false,true,false,false],merge(make_dict(F["args"],F["pars"], merge(model_params, Dict(params))))...)
