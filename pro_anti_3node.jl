...
README = """
This code base defines the three node pro-anti network. All functions are designed to be equivalent to the four node networks defined in pro_anti.jl
code by alex piet 07/05/2018

    FUNCTION                UPDATED FOR 3NODE?      TESTED WITH 3NODE?      COMMENTS
    ...
    MAJOR CHANGES
    removed input parameter "right_light_excitation" and replaced it with "target_period_anti_excitation"
...
"""
    plot_PA_3node(t, U, V; fignum=1, clearfig=true, rule_and_delay_period=1, target_period=1, 
        post_target_period=1, fontsize=20, color_list = [0 0 1; 1 0 0 ; 1 0.5 0.5 ],
        singleton_color = [0 0.5 0], linestyle = "-",
        plottables = ["V", "U", "V[1,:]-V[3,:]"], ylabels = ["V", "U", "Pro_R - Pro_L"],
...
plot V vs time, U versus time, and V[1,:]-V[3,:] versus time in three vertically
arranged subplots.
...
- U     a 3-by-length(t) matrix representing activities of Pro_L, Anti, Pro_R units
        May also be a 3-by-length(t)-by-ntrials matrix, in which case all trials are plotted on top of each other
...
- plottables   A vector of strings. Each of these strings indicates something to 
            ...
            and "t" are instantiated to have the values passed to `plot_PA_3node()`. Thus 
            plottables=["V", "V[1,:]-V[3,:]"] indicates that two axes should be used, on
            the first one V will be plotted, on the second V[1,:]-V[3,:].
...
- ax_set    If passed, should be a vector of axis handles, equal in length to 
            ...
            on which V, U, and V[1,:] - V[3,:] will be plotted. In this backwards compatibility
...
"""
function plot_PA_3node(t, U, V; fignum=1, clearfig=true, rule_and_delay_period=1, target_period=1, 
    post_target_period=1, fontsize=20, color_list = [0 0 1; 1 0 0 ; 1 0.5 0.5],
    singleton_color = [0 0.5 0],  linestyle = "-",
    plottables = ["V", "U", "V[1,:]-V[3,:]"], ylabels = ["V", "U", "Pro_R - Pro_L"],
    ylims = [[-0.02, 1.02], nothing, [-1.02, 1.02]], xlims = nothing, plot_Us = nothing,
    ...
    for i=1:nplots
        ...
        for k=1:size(V,3)  # loop over trials
            vardict = Dict("V"=>V[:,:,k], "U"=>U[:,:,k], "t"=>t,
                "decis"=>V[1,:,k]-V[3,:,k], 
                "rule"=>0.5*(V[1,:,k]+V[3,:,k]-V[2,:,k]))   # variables for plottables evaluation
...
"""
parsed_times = parse_opto_times_3node(opto_times, model_params)
...
> parse_opto_times_3node(opto_times, model_params)

7.38906   3
 6         8

"""
function parse_opto_times_3node(opto_times2, model_params)
...
# :target_period_anti_excitation   => 0.1,    # input added to the Anti unit during the target_period
...
"""
input, t, nsteps = make_input_3node(trial_type; dt=0.02, nderivs=0, difforder=0, constant_excitation=0.19, anti_rule_strength=0.1, 
    pro_rule_strength=0.1, target_period_excitation=1, target_period_anti_excitation=0.1, right_light_pro_extra=0, 
    rule_and_delay_period=0.4, target_period=0.1, post_target_period=0.4, const_pro_bias=0,
    other_unused_params...)
"""
function make_input_3node(trial_type; dt=0.02, nderivs=0, difforder=0, constant_excitation=0.19, anti_rule_strength=0.1, 
    pro_rule_strength=0.1, target_period_excitation=1, target_period_anti_excitation=0.1, right_light_pro_extra=0, 
    rule_and_delay_period=0.4, target_period=0.1, post_target_period=0.4, const_pro_bias=0,
    other_unused_params...)
    ...
    if FDversion() >= 0.6
        ...
        target_period_anti_excitation, right_light_pro_extra, rule_and_delay_period, target_period, post_target_period,
        const_pro_bias)
    end
    ...
    if FDversion() < 0.6
        input = constant_excitation + ForwardDiffZeros(3, nsteps, nderivs=nderivs, difforder=difforder)
    else
        input = constant_excitation + zeros(get_eltype(varbag), 3, nsteps)
    end
    
    if trial_type=="Anti"
        input[2, t.<rule_and_delay_period] += anti_rule_strength
    elseif trial_type=="Pro"
        input[[1,3], t.<rule_and_delay_period] += pro_rule_strength
    else
        error("make_input_3node: I don't recognize input type \"" * trial_type * "\"")
    end
    ...
    if VERSION.major + 0.1*VERSION.minor < 0.6
        ...
        input[2,     (rule_and_delay_period.<=t) & (t.<rule_and_delay_period+target_period)] += target_period_anti_excitation
...
proVs, antiVs, pro_fullV, anti_fullV, opto_fraction, pro_input, anti_input = 
        run_ntrials_3node(nPro, nAnti; plot_list=[], selectize =  "true",
        start_pro=[-0.5,-0.5,-0.5], start_anti=[-0.5,-0.5,-0.5],
        profig=1, antifig=2, clearfig=true, ax_set = nothing, hit_linestyle="-", err_linestyle="-",
        opto_units = 1:3, nderivs=0, difforder=0, symmetrized_W=true, model_params...)
...
- symmetrized_W     If true, model_params must include parameters :vW, :hW, :sW, and :dW, from 
                    which the 3-by-3 matrix is built. Connections between Pro units are assumed to be the same as 
...
- start_pro    A 3-by-1 vector of starting U values on Pro trials for the 3 units

- start_anti   A 3-by-1 vector of starting U values on Anti trials for the 3 units
...
- ax_set       If passed, should be a vector with two elements, the first for the Pro
               ...
               whose values are corresponding axis handles, to be passed to `plot_PA_3node()`
...
- model_params   Further optional params, will be passed onto forwardModel() (e.g., opto times, although
                opto_times is first passed through parse_opto_times_3node())
...
- proVs    final V for the three units on Pro trials

- antiVs   final V for the three units on Anti trials

- pro_fullVs   all Vs, across all times, for all Pro trials (3-by-nsteps-by-nPro)

- anti_fullVs  all Vs, across all times, for all Anti trials (3-by-nsteps-by-nAnti)


# EXAMPLE
```jldoctest
proVs, antiVs, pro_fullV, anti_fullV, opto_strength, pro_input, anti_input = 
    run_ntrials_3node(nPro, nAnti; plot_list=[1:5;], 
    plottables = ["V", "(V[1,:]+V[3,:]) - (V[2,:])", "V[1,:]-V[3,:]"],
    ylabels = ["V", "Pro - Anti rule encoding", "Pro_R - Pro_L"],
    ylims = [[-0.02, 1.02], nothing, nothing], err_linestyle="--",
    my_params...)
```
"""
function run_ntrials_3node(nPro, nAnti; plot_list=[], start_pro=[-0.5,-0.5,-0.5], start_anti=[-0.5,-0.5,-0.5],
    profig=1, antifig=2, clearfig=true, ax_set = nothing, hit_linestyle="-", err_linestyle="-",
    opto_units = 1:3, nderivs=0, difforder=0, selectize=true, symmetrized_W=true, model_params...)
    ...
    pro_input,  t, nsteps = make_input_3node("Pro" ; nderivs=nderivs, difforder=difforder, model_params...)
    ...
    if FDversion()<0.6
        proVs  = ForwardDiffZeros(3, nPro, nderivs=nderivs, difforder=difforder)
        antiVs = ForwardDiffZeros(3, nAnti, nderivs=nderivs, difforder=difforder)
    else
        proVs  = zeros(get_eltype(varbag), 3, nPro)
        antiVs = zeros(get_eltype(varbag), 3, nAnti)
    end
    proVall  = zeros(3, nsteps, nPro);
    antiVall = zeros(3, nsteps, nAnti);
    proUall  = zeros(3, nsteps, nPro);
    antiUall = zeros(3, nsteps, nAnti);
    # --- PRO ---
    ...
    for i=1:nPro
        ...
        sDict = Dict("t"=>t, "V"=>proVall[:,:,i], "U"=>proUall[:,:,i], "Vend"=>Vend,
            "rule" =>0.5*(proVall[1,:,i]+proVall[3,:,i]-proVall[2,:,i]),
            "decis"=>proVall[1,:,i] - proVall[3,:,i],
            "ispro"=>true, "isanti"=>false, 
            "ishit"=>Vend[1]>=Vend[3], "iserr"=>Vend[1]<Vend[3])
    ...
    if hit_linestyle==err_linestyle && hit_linestyle!=""
        ...
        plot_PA_3node(t, get_value(proUall[:,:,uI]), get_value(proVall[:,:,uI]); clearfig=false,
    ...
    for i=1:nAnti
        ...
        sDict = Dict("t"=>t, "V"=>antiVall[:,:,i], "U"=>antiUall[:,:,i], "Vend"=>Vend,
            "rule" =>0.5*(antiVall[1,:,i]+antiVall[3,:,i]-antiVall[2,:,i]),
            "decis"=>antiVall[1,:,i] - antiVall[3,:,i],
            "diag"=>0.5*(antiVall[1,:,i]+antiVall[3,:,i] - antiVall[2,:,i]),
    ...
    if hit_linestyle==err_linestyle && hit_linestyle!=""
        uI = find(plt_vector .& sel_vector)
        plot_PA_3node(t, get_value(antiUall[:,:,uI]), get_value(antiVall[:,:,uI]); clearfig=false,
...
cost, cost1s, cost2s, hP, hA, dP, dA, hBP, hBA, [proValls, antiValls, opto_fraction, pro_input, anti_input] = 
    JJ_3node(nPro, nAnti; pro_target=0.9, anti_target=0.7, 
...
- opto_periods  Each row is the [start_time, end_time] for an opto condition. These follow the rules
                of `parse_opto_times_3node()`: they can be arbitrary Julia expressions involving the terms 
...
- model_params              Any remaining keyword-value params are passed as is on to run_ntrials_3node 
...
function JJ_3node(nPro, nAnti; pro_target=0.9, anti_target=0.7, 
    opto_targets = Array{Float64}(0,2), opto_periods = Array{Float64}(0,2), 
    model_details = false,
    theta1=0.025, theta2=0.035, cbeta=0.003, verbose=false, verbose_file = STDOUT,
    pre_string="", zero_last_sigmas=0, seedrand=NaN, 
    ...
    if target_periods==nothing        
        ...
        @printf(ostr, "\n\nWARNING: JJ_3node() was not given a target_periods parameter, I will *IGNORE* the\n")
            ...
            for j in target_periods
                for k = post_target_periods
                    ...
                    proVs, antiVs, proVall, antiVall, opto_fraction,pro_input,anti_input =
                        run_ntrials_3node(nPro, nAnti; plot_list=my_plot_list, 
...
"""
model_params, F, nPro, nAnti = load_run_3node(run_name; farmdir="FarmFields")

Loads a run and sets everything into a self-contained model_params so that you could then run it directly:

    JJ_3node(model_params[:nPro], model_params[:nAnti]; model_params...);
...
sets :start_pro and :start_anti to a default of [-0.5, -0.5, -0.5]
...
"""
function load_run_3node(run_name; farmdir="FarmFields")

    default_U_start = [-0.5, -0.5, -0.5]
    ...
    if ~haskey(model_params, :target_periods)
        model_params[:target_periods] = 0.1  # Use the JJ_3node() default
        model_params[:target_period]  = 0.1  # Use the JJ_3node() default
        @printf("\n\nWARNING: :target_periods was not specified, JJ_3node() would ignore :target_period and\n")
        @printf("would use a default of :target_periods=>[0.1].  Setting that explicitly here, no warning from JJ_3node()\n")
