...
export oneDMinimizer, constrainedParabolicMinimization, testMe

"""
function x, cost, iters_used, last_Delta_x = oneDMinimizer(seed, func;
        tol=1e-5, maxiter=100, start_eta=0.1, verbose=false)
...
- verbose      If true, reports to stdout after every iteration
...
"""
function oneDMinimizer(seed, func; tol=1e-5, maxiter=100, start_eta=0.1,
    verbose=false)
    eta = start_eta;
    lambdavec = [seed]
    ...
    for i=1:maxiter
        ...
        if verbose
            println("$i: cost=$cost lambda=$(lambdavec[1])")
        end
    end
...
"""
function x, J, lambdas, costs, niters, Dlambda = constrainedParabolicMinimization(H, G, r; tol=1e-6,
...
"""
function constrainedParabolicMinimization(H, G, r; tol=1e-6, min_only=true,
    ...
    """
    function x_of_lambda(lambda)
        return inv(H - lambda*Diagonal(ones(size(H,1))))*(-G)
    end
    ...
    for m=1:max_efactor_tries
        ...
        g = append!(prepend!(findall(diff(sign.(diff(costs))) .> 0.99), [1]), [length(lambdas)])
        if verbose
            println("cpm: g (candidate indices) are : ", g);
            println("cpm: and their corresponding costs are : ", costs[g])
            println("cpm: and their corresponding lambdas are : ", lambdas[g]);
            println("cpm: the minimum cost was : ", minimum(costs[g]))
        end
        ...
        while mytol > minimum_tol
            ...
            for i in [1:length(g);]
                ...
                oneDMinimizer(lambdas[g[i]], x -> q(x[1], r), start_eta=1, tol=mytol, maxiter=maxiter)
            end

            # Eliminate any lambdas where x'*x doesn't match our desired value r
            I = findall(costs_out .< tol)
        ...
        if verbose
            println("$m : After searching for lambdas with efactor=$efactor, we found these : ", lambdas_out)
        end
    ...
    I = setdiff(1:length(lambdas_out), findall(diff(lambdas_out) .< tol))
    lambdas_out = lambdas_out[I]; costs_out = costs_out[I];
    niters_out  = niters_out[I];  Dlambda_out = Dlambda_out[I]
...
@doc """
    ...
    Run this function (no parameters, no returns) to test constrainedParabolicMinimization
""" function testMe()
    ...
    p = constrainedParabolicMinimization(H, g, r0)[1]


    xax = -11:0.1:11
    yax = -10:0.1:10
    X = repeat(xax', length(yax), 1)
    Y = repeat(yax, 1, length(xax))
    ...
    axis("scaled")
    xlabel("x"); ylabel("y")
    title("magenta line is constant radius, red dot is the minimum\n"*
        "constrained to be on or less than that radius")

    theta = atan(p[2]/p[1])
    func([r0*cos(theta), r0*sin(theta)])

    e = -0.001
    println("Moving $e along radius changes cost by $(func([r0*cos(theta+e),
        r0*sin(theta+e)]) - func([r0*cos(theta), r0*sin(theta)]))")
    e = 0.001
    println("Moving $e along radius changes cost by $(func([r0*cos(theta+e),
        r0*sin(theta+e)]) - func([r0*cos(theta), r0*sin(theta)]))")
