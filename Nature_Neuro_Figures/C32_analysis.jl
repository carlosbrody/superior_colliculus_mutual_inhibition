# This file is a collection of scripts, notes, and functions for analyzing the C32 model.
# It has mutated a bit, so its less well documented than it should be.
# My apologies. Alex Piet 12/2018
#
# These are some functions for doing the basic analysis
...
cluster_farms("C32"; farmdir="MiniC32", threshold = -0.0001)  # we didnt use this in the end
cp compute_clustering/MarinoCode_C32_MiniC32.jld MiniC32_C32_clusters.jld # we didnt use this in the end
examples, cluster_example_trajectories("C32", "MiniC32"; threshold=-0.0001) # This generates a bunch of examples, used in a lot of analyses
unilateral, test_farm_unilateral("C32","MiniC32";threshold=-0.0001) # we didnt use this in the end, and im pretty sure its buggy

# connectivity matrix, for references
...
# see C32_index_analysis.jl for the dprime plots in Model figure 1 e/f
...
# this makes Supplementary model figure 2, which is a nice histogram of all parameters
...
# Load cluster identities, if you are using them
#cp compute_clustering/MarinoCode_C32_MiniC32.jld MiniC32_C32_clusters.jld # just need to run this once
cluster_info    = load("MiniC32_C32_clusters.jld");
cluster_ids     = cluster_info["idx"];

# scatter parameters. This makes the nice figures that ended up in the paper Model figure 2 panel C.
