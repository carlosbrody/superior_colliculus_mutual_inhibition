...
:vW     =>                   [mypars[:vW],                       [-3,    3],  [-3,   3]], 
:hW     =>                   [mypars[:hW],                       [-3,    3],  [-3,   3]],
:dW     =>                   [mypars[:dW],                       [-3,    3],  [-3,   3]],
:sW     =>                   [mypars[:sW],                       [0,     3],  [0,    3]],
:sigma  =>                   [0.11,                              [0.1,   2],  [-2,   2]],
:constant_excitation      => [mypars[:constant_excitation],      [-30,  30],  [-30, 30]], 
:target_period_excitation => [mypars[:target_period_excitation], [-30,  30],  [-30  30]],
:right_light_excitation   => [mypars[:right_light_excitation],   [-30,  30],  [-30, 30]],
:const_pro_bias           => [mypars[:const_pro_bias],           [-30,  30],  [-30, 30]],
:opto_strength            => [mypars[:opto_strength],            [0,     1],  [0,    1]],
:pro_rule_strength        => [mypars[:pro_rule_strength],        [0,    30],  [0,   30]],
:anti_rule_strength       => [mypars[:anti_rule_strength],       [0,    30],  [0,   30]],
)
...
Farm C18: Like C17/C16, with opto_strength and pro_rule_stregth and anti_rule_strength as 
trainable parameters,  with reasonable bounds on the appropriate parameters. 
The initial search box is expanded to equal to the entire bound box
"""
...
while true
    ...
    presearchiter = 50; # the initial seach
    ...
    prefunc1 =  (;params...) -> JJ(presearchiter, presearchiter; verbose=false, verbose_file=report_file, 
    ...
    try
        ...
        pars3, traj3, cost3, cpm_traj3, ftraj3 = bbox_Hessian_keyword_minimization(seed, 
            args, bbox, prefunc1, start_eta = 0.01, tol=1e-12, verbose_file=report_file,
        ...
        # check to see if we do a primary search
        INITIAL_SEARCH_GOOD = all(abs(extra_pars[:opto_targets] - [hBP hBA]) .<= .1);

        if INITIAL_SEARCH_GOOD
            presearch = Dict("README"=>README, "nPro"=>mypars[:nPro], "nAnti"=>mypars[:nAnti], 
        ...
        append_to_file(report_file, @sprintf("\n\n ****** writing to file %s *******\n\n", myfilename))
        if INITIAL_SEARCH_GOOD
            append_to_file(report_file, @sprintf("\n\n ****** Initial search good, ran main search %s *******\n\n", myfilename))   
        else
            append_to_file(report_file, @sprintf("\n\n ****** Initial search bad, did not run main search %s *******\n\n", myfilename))
        end
        ...
        save(myfilename, Dict("README"=>README, "nPro"=>mypars[:nPro], "nAnti"=>mypars[:nAnti], 
            ...
            "pars3"=>pars3, "traj3"=>traj3, "cost3"=>cost3, "cpm_traj3"=>cpm_traj3, "ftraj3"=>ftraj3,
            "cost"=>cost, "cost1s"=>cost1s, "cost2s"=>cost2s, "bbox"=>bbox, 
            "hP"=>hP, "hA"=>hA, "dP"=>dP, "dA"=>dA, "hBP"=>hBP, "hBA"=>hBA,"presearch"=>presearch))
