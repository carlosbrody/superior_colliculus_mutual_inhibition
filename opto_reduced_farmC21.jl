include("setup_C21.jl")



README = """

Farm C21: In a single file, do a pre-search with 50 trials/condition, then if it passes
criteria, train further at 1600 trials/condition; two rule_and_delay_periods, no post_target_period
...
fbasename = farmdir * "/" * "farm_C21_" * chomp(readstring(`hostname`)) * "_"
...
just_testing = false; if just_testing   # 
    ...
    extra_pars[:binarized_delta_threshold] = 0.99     # average frac correct must be within this of target
...
while true
    ...
    try
        ...
        Pro_targ  = extra_pars[:opto_targets][:,1]*ones(1,size(hBPA,2)) # copy for number of period durations
        Anti_targ = extra_pars[:opto_targets][:,2]*ones(1,size(hBAA,2))

        append_to_file(report_file, @sprintf("mean diff = %g\n", mean(abs.([Pro_targ Anti_targ] - [hBPA hBAA]))))
        append_to_file(report_file, @sprintf("all(hBPA .> hBAA) = %s\n", all(hBPA .> hBAA)))
        print_vector_g(report_file, hBPA); print_vector_g(report_file, hBAA)
        append_to_file(report_file, @sprintf("\nall(hBAA[2,:] .<= hBAA[1,:] - extra_pars[:anti_perf_delta]) = %s\n", 
                                             all(hBAA[2,:] .<= hBAA[1,:] - extra_pars[:anti_perf_delta])))
        append_to_file(report_file, @sprintf("all(hBAA[2,:] .<= hBAA[3,:] - extra_pars[:anti_perf_delta]) = %s\n", 
                                             all(hBAA[2,:] .<= hBAA[3,:] - extra_pars[:anti_perf_delta])))


        if mean(abs.([Pro_targ Anti_targ] - [hBPA hBAA])) < extra_pars[:binarized_delta_threshold]  && 
