...
function JJ_opto_delay(nPro, nAnti; opto_targets=[0.9 0.7], theta1=0.025, theta2=0.035, cbeta=0.003, verbose=false, pre_string="", zero_last_sigmas=0, seedrand=NaN, delay_period = 0, rule_and_delay_periods = [0.4], target_periods = [0.1], post_target_periods = [0.5], opto_periods = [-1 -1],opto_strength=1, nderivs=0, difforder=0, model_params...) #set opto defaults!
    ...
    for i in rule_and_delay_periods
        for j in target_periods
            for k = post_target_periods
                ...
                my_params = make_dict(["rule_and_delay_period", "target_period", "post_target_period","opto_period","opto_strength","delay_period"], [i, j, k, opto_periods[kk,:], opto_strength,delay_period], Dict(model_params))
    
                # print("model params is " ); print(model_params); print("\n")
                proVs, antiVs = run_ntrials_opto_delay(nPro, nAnti; nderivs=nderivs, difforder=difforder, my_params...)
...
function make_opto_input_delay(nsteps; dt = 0.02, opto_strength=1, opto_period=[-1 1],nderivs=0,difforder=0, other_unused_params...);
...
#        println()
        ...
        end_target = other_unused_params[:rule_and_delay_period]+other_unused_params[:delay_period]+other_unused_params[:target_period];
        end_delay =  other_unused_params[:rule_and_delay_period]+other_unused_params[:delay_period]; 
        if opto_period[1]==100
            opto_period[1] = end_rd;
        elseif opto_period[1]==200
            opto_period[1] = end_target;
        elseif opto_period[1]==300
            opto_period[1] = end_delay;
        end        
        if opto_period[2]==100
            opto_period[2] = end_rd;
        elseif opto_period[2]==200
            opto_period[2] = end_target;
        elseif opto_period[2]==300
            opto_period[2] = end_delay;
        end        
...
function run_ntrials_opto_delay(nPro, nAnti; plot_list=[], nderivs=0, difforder=0,start_pro=[-0.3, -0.7, -0.7, -0.3], start_anti=[-0.7, -0.3, -0.3, -0.7],  opto_periods=[-1 -1],model_params...)
    pro_input,  t, nsteps = make_input_delay("Pro" ; model_params...);
    anti_input, t, nsteps = make_input_delay("Anti"; model_params...);
    #### make opto fraction vector
    opto_fraction = make_opto_input_delay(nsteps; nderivs=nderivs,difforder=difforder, model_params...);
...
function make_input_delay(trial_type; dt=0.02, nderivs=0, difforder=0, constant_excitation=0.19, anti_rule_strength=0.1, 
    pro_rule_strength=0.1, target_period_excitation=1, right_light_excitation=0.5, right_light_pro_extra=0, 
    rule_and_delay_period=0.4, target_period=0.1, post_target_period=0.4, const_pro_bias=0, delay_period=0,
    other_unused_params...)

    T = rule_and_delay_period +delay_period+ target_period + post_target_period
    t = 0:dt:T
    nsteps = length(t)
    ...
    input[:,     (rule_and_delay_period+delay_period.<=t) & (t.<rule_and_delay_period+delay_period+target_period)] += target_period_excitation
    input[1:2,   (rule_and_delay_period+delay_period.<=t) & (t.<rule_and_delay_period+delay_period+target_period)] += right_light_excitation
    input[1,     (rule_and_delay_period+delay_period.<=t) & (t.<rule_and_delay_period+delay_period+target_period)] += right_light_pro_extra
