...
function make_opto_input(nsteps; dt = 0.002, opto_strength=1, opto_period=[-1 1],nderivs=0,difforder=0, other_unused_params...);
...
function run_ntrials_opto(nPro, nAnti; plot_list=[], nderivs=0, difforder=0, opto_periods=[-1 -1],model_params...)
    ...
    weights=[sW vW dW hW; vW sW hW dW; dW hW sW vW; hW dW vW sW];
    ...
    proVsall  = zeros(4, 301,nPro)
    antiVsall = zeros(4, 301,nAnti)
    ...
    return proVs, antiVs, proVsall, antiVsall, opto_fraction, pro_input, anti_input, weights
end

function forwardModel_opto(startU, opto_fraction; dt=0.002, tau=0.02, nsteps=100, input=[], noise=[], W=[0 -5;-5 0], 
    ...
    for i=2:nsteps
        ...
        # println(dt/tau*g_leak*(U_rest -U[:,i-1]))
        # println(dt/tau*W*V[:,i-1])
        # println((dt/tau)*dUdt)
        # println("diofa")
        # error("puppa")
