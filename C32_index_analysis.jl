...
for i=1:size(ex,1)
    for j=1:size(ex,6)
        ...
        choice_dex[i,j+size(ex,6),:] = (exS[i,1,2,1,:,j] - exS[i,1,2,4,:,j]) ;
    end
...
for i=1:size(ex,1)
    for j=1:size(ex,6)
        ...
        rule_dex[i,j+size(ex,6),:] = 0.5.*(ex[i,1,2,1,:,j] + ex[i,1,2,4,:,j]) - 0.5.*(ex[i,1,2,2,:,j] + ex[i,1,2,3,:,j]);
    end
    prodex  = collect(1:1:size(ex,6));
    antidex  = collect(size(ex,6)+1:1:size(ex,6)*2);
