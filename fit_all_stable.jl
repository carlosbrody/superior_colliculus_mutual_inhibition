...
#
#
# Both of these inputs seem to be giving the wrong opto times!
:opto_periods  => [0 0;0 "exp(trial_end)";0 "exp(target_start)"; "exp(target_start)" "exp(trial_end)"],
# :opto_periods  => [0 0;0 20;0 100; 100 20], 
#
#
# 
...
for cb in cbetas                # Iterate over beta values, if there are multiple
    ...
    t_standard_func =  (;params...) -> JJ(model_params[:nPro], model_params[:nAnti]; rule_and_delay_periods=rule_and_delay_periods, theta1=model_params[:theta1], theta2=model_params[:theta2], post_target_periods=post_target_periods, seedrand=sr, cbeta=cb, verbose=false, merge(model_params, Dict(params))...)
