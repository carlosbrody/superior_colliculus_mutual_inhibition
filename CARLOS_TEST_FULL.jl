...
test_func =  (;params...) -> JJ_opto_plot(100,100; rule_and_delay_periods=F["rule_and_delay_periods"], theta1=model_params[:theta1], theta2=model_params[:theta2], post_target_periods=F["post_target_periods"], seedrand=F["test_sr"], cbeta=F["cb"], verbose=true,plot_conditions=[false,true,false,false,false],merge(make_dict(F["args"],F["pars"], merge(model_params, Dict(params))))...)
...
figure()
plot([1,2,3,4,5],opto_hitsP,color="red")
plot([1,2,3,4,5],opto_hitsA,color="blue")
...
p_err = round(1.96*sqrt((1/1000)*opto_hitsP.*(1-opto_hitsP))*1000)/1000
a_err = round(1.96*sqrt((1/1000)*opto_hitsA.*(1-opto_hitsA))*1000)/1000
for i=1:5
plot([i,i], opto_hitsP[i]+[p_err[i], -p_err[i]],color="red")
plot([i,i], opto_hitsA[i]+[a_err[i], -a_err[i]],color="blue")
end
ylabel("hits %")
xlabel("condition")
