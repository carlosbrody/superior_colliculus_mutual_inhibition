...
function JJ_marino(nPro, nAnti; pro_target=0.9, anti_target=0.7, 
    ...
    differenzepro = ForwardDiffZeros(nruns, nPro, nderivs=nderivs, difforder=difforder)
    differenzeanti = ForwardDiffZeros(nruns, nPro, nderivs=nderivs, difforder=difforder)

    hitpro = ForwardDiffZeros(nruns, nPro, nderivs=nderivs, difforder=difforder)
    hitanti = ForwardDiffZeros(nruns, nPro, nderivs=nderivs, difforder=difforder)


    opro = ForwardDiffZeros(nruns,1, nderivs=nderivs, difforder=difforder)
    oanti = ForwardDiffZeros(nruns,1, nderivs=nderivs, difforder=difforder)
    ...
    for i in rule_and_delay_periods
        ...
        for j in target_periods
            ...
            for k = post_target_periods
                ...
                differenzepro[n,:]=(proVs[1,:,]-proVs[4,:]);
                differenzeanti[n,:]=(antiVs[4,:,]-antiVs[1,:]);

                hitpro[n,:]=hitsP;
                hitanti[n,:]=hitsA;


                opro[n] = sum(proVs[1,:] .>= proVs[4,:,])/nPro;
                oanti[n] = sum(antiVs[4,:] .>=  antiVs[1,:,])/nAnti;
    ...
    return opro,oanti,differenzepro,differenzeanti,hitpro,hitanti
end



function JJ_opto_marino(nPro, nAnti; opto_targets=[0.9 0.7], theta1=0.025, theta2=0.035, cbeta=0.003, verbose=false, pre_string="", zero_last_sigmas=0, seedrand=NaN, rule_and_delay_periods = [0.2], target_periods = [0.1], post_target_periods = [0], opto_periods = [-1 -1],opto_strength=1, nderivs=0, difforder=0, model_params...) #set opto defaults!
    ...
    resP = zeros(4,301,nPro,size(opto_periods)[1], nruns_each);
    resA = zeros(4,301,nAnti,size(opto_periods)[1], nruns_each);
    opto_fractions = zeros(4,301,size(opto_periods)[1], nruns_each);
    pro_inputs = zeros(4,301,size(opto_periods)[1], nruns_each);
    anti_inputs = zeros(4,301,size(opto_periods)[1], nruns_each);
    weightss = zeros(4,4,size(opto_periods)[1], nruns_each);
    ...
    for i in rule_and_delay_periods
        for j in target_periods
            for k = post_target_periods
                ...
                # print("model params is " ); print(model_params); print("\n")
                proVs, antiVs, proVsall, antiVsall, opto_fraction, pro_inputs, anti_inputs, weightss = run_ntrials_opto(nPro, nAnti; nderivs=nderivs, difforder=difforder, my_params...)
                ...
                resP[:,:,:,kk,nopto] = proVsall;
                resA[:,:,:,kk,nopto] = antiVsall;

                opto_fractions[:,:,kk,nopto]=opto_fraction;
...
#    end 
    ...
    return hBP,hBA,resP,resA, opto_fractions, pro_inputs, anti_inputs, weightss
end
